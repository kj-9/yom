...
2026-09-05にレスポンシブ配置とモバイル文書ドロワーを実装し、2026-09-06に
表示設定のポップオーバー／シートを完成しました。320〜1,440pxの配置、低い画面での
設定操作、dev/staticの画像一致、フォーカスとスクロールの復帰、全配色のaxe監査を
検証済みです。サイト既定値のSSR・Resetへの反映とシステムテーマの追従も修正しました。
